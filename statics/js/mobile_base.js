$(document).ready(function(){ 
    if(!isAppWeb()){
        $("#foot-btn").html('<div class="sport_down_bar"></div><div class="sport_down clearfix"><div class="fl down_logo"><img src="/statics/images/logo.png"></div><div class="fl download_font"><p class="appName">快乐跑</p><p class="titlt_sub">让奔跑更有趣！</p></div><div class="fr clearfix foot_operate"><a class="fl open_app" href="http://a.app.qq.com/o/simple.jsp?pkgname=com.csda.sportschina">马上下载</a><a class="fl close_foot" href="javascript:$(\'#foot-btn\').hide();"></a></div></div>')
    }
}); 
...
function Call_NativeApp(module,type,value) {
      type = type || 2;  
      value = value || ''; 
      setupWebViewJavascriptBridge(function (bridge) {  
         bridge.callHandler('app_call', {'m': module,'t':type,'v':value}, function (responseData) {  

         });              
     })
}

function BuildSchema(module,value,pars) {
      value = value || '';  
      pars = pars || ''; 
}
function wx_share(appinfo,shareinfo) {
    wx.config({
    debug: false,
    appId: appinfo.appid,
...
function isAppWeb(version) {
  var userAgent = window.navigator.userAgent;
  userAgent = userAgent.toLocaleLowerCase();
  if (userAgent.indexOf("sportchina") != -1) {
    return true
  }
  return false
}
...
function isIosWeb(){var userAgent=window.navigator.userAgent;userAgent=userAgent.toLocaleLowerCase();if(userAgent.indexOf("iphone")!=-1||userAgent.indexOf("ipad")!=-1){return true}return false}
function isAndroidWeb(){var userAgent=window.navigator.userAgent;userAgent=userAgent.toLocaleLowerCase();if(userAgent.indexOf("android")!=-1){return true}return false}
function isWeixin(){var userAgent=window.navigator.userAgent;userAgent=userAgent.toLocaleLowerCase();if(userAgent.indexOf("micromessenger")!=-1){return true}return false}
